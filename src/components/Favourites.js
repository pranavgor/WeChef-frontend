...
const columns = [
    {
        ...
        options: {
            ...
            return (
                    <Avatar src={val} sx={{ height:'100px', width:'100px'}}></Avatar>
                )
            },
        }
    },
    ...
    },
    {
        ...
        label: "Favourite",
        options: {
            setCellProps: () => ({ style: { marginLeft:'10px',minWidth: colWidth, maxWidth: colWidth }}),
            filter: false,
            sort: false,
            customBodyRender: (val) => {
            return (
...
function Favourites() {
    ...
    useEffect(()=>{
        ...
        axios.get("http://localhost:5000/recipes/self/fav", { headers: headers, method: 'GET'})
        .then(res => {
            if (res.status === 200) {
                let ans = []
                for(var i = 0; i < res.data.data.length; i++){
                    ...
                    let temp = []
                    temp.push(res.data.data[i].image)
                    temp.push(res.data.data[i].name)
                    temp.push(res.data.data[i].ingredients)
                    temp.push(res.data.data[i].tag)
                    temp.push(res.data.data[i]._id)
                    temp.push([res.data.data[i]._id, true])
                    ans.push([...temp])
                    temp = []
                }
                setRecipes(ans)
